...
public class ItemBigDoor extends Item {
    ...
    public static void placeDoor(final World worldIn, final BlockPos pos, final EnumFacing facing, final Block door, boolean isRightHinge) {
        ...
        final int i = (worldIn.getBlockState(blockpos).isNormalCube() ? 1 : 0) + (worldIn.getBlockState(blockpos.up()).isNormalCube() ? 1 : 0);
        final int j = (worldIn.getBlockState(blockpos1).isNormalCube() ? 1 : 0) + (worldIn.getBlockState(blockpos1.up()).isNormalCube() ? 1 : 0);
        //int k = (worldIn.getBlockState(blockpos2).isNormalCube() ? 1 : 0) + (worldIn.getBlockState(blockpos2.up(2)).isNormalCube() ? 1 : 0);
        final boolean flag = worldIn.getBlockState(blockpos).getBlock() == door || worldIn.getBlockState(blockpos.up()).getBlock() == door;
        final boolean flag1 = worldIn.getBlockState(blockpos1).getBlock() == door || worldIn.getBlockState(blockpos1.up()).getBlock() == door;
        ...
        final IBlockState iblockstate = door.getDefaultState().withProperty(BigDoor.FACING, facing).withProperty(BigDoor.HINGE, isRightHinge ? BigDoor.EnumHingePosition.RIGHT : BigDoor.EnumHingePosition.LEFT).withProperty(BigDoor.POWERED, Boolean.valueOf(flag3)).withProperty(BigDoor.OPEN, Boolean.valueOf(flag3));
