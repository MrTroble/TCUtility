 ...
 * auf die 1.14.4-{@link net.minecraft.fluid.FlowingFluid}-API:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>1.14.4-Mappings: {@code spread} -> {@code flowAround},
 * {@code getNewLiquid} -> {@code calculateCorrectFlowingState},
 * {@code spreadTo} -> {@code flowInto}, {@code canSpreadTo} -> {@code canFlow},
 * {@code getDropOff} -> {@code getLevelDecreasePerBlock}; FluidState heisst
 * hier {@link IFluidState}.
 */
public abstract class TCUpwardFlowingFluid extends ForgeFlowingFluid {
    ...
    @Override
    protected IFluidState calculateCorrectFlowingState(final IWorldReader world,
            final BlockPos pos, final BlockState state) {
        final IFluidState below = world.getFluidState(pos.down());
        ...
        for (final Direction dir : Direction.Plane.HORIZONTAL) {
            final IFluidState neighbor = world.getFluidState(pos.offset(dir));
    ...
    @Override
    protected void flowAround(final IWorld world, final BlockPos pos, final IFluidState state) {
        if (state.isEmpty()) {
            return;
        }
        ...
        final IFluidState aboveFluid = world.getFluidState(abovePos);
        final IFluidState upFlow = this.getFlowingFluidState(COLUMN_LEVEL, false);
        ...
        final IFluidState sideFlow = this.getFlowingFluidState(sideLevel, false);
        for (final Direction dir : Direction.Plane.HORIZONTAL) {
            final BlockPos sidePos = pos.offset(dir);
            final BlockState sideState = world.getBlockState(sidePos);
            final IFluidState sideFluid = world.getFluidState(sidePos);
